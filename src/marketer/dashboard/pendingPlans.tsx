...
const PendingPlans = ({ customerData }: { customerData?: any }) => {
  const dispatch = useDispatch<AppDispatch>();
  const pagination = useSelector(selectPendingPlansPagination);
  const navigate = useNavigate();

  const handlePageChange = (page: number) => {
    dispatch(setPendingPlansCurrentPage(page));
                ...
                >
                  ...
                  <td className="pr-2 whitespace-nowrap">
                    <div className="pb-8 font-[325] text-dark text-sm">
                      {formatAsNaira(row.totalAmount) ?? "N/A"}
                    </div>
                  </td>
                  <td className="pr-2 whitespace-nowrap">
                    <div className="pb-8 font-[325] text-dark text-sm">
                  ...
                  <td className="pr-2 whitespace-nowrap">
                    <div className="pb-8 font-[325] text-dark text-sm">
                      {formatAsNaira(row.remaining_balance) ?? "N/A"}
                    </div>
                  </td>
                  <td className="pr-2 whitespace-nowrap">
                    <div className="pb-8 font-[325] text-dark text-sm">
