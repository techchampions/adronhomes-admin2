...
export const fetchMarketerDashboardpersonnel = createAsyncThunk<
  MarketerDashboardResponse,
  { currentpage: number; marketerId: string }, // Added marketerId parameter
  {
    state: RootState;
    rejectValue: ErrorResponse;
  }
>(
  "dashboard/fetchMarketerDashboardpersonnel",
  async ({ currentpage, marketerId }, { rejectWithValue }) => {
    ...
    try {
      const response = await api.get<MarketerDashboardResponse>(
        `${BASE_URL}/api/admin/marketer-data/${marketerId}`, // Updated URL with marketerId
        {
          headers: {
      ...
      if (axiosError.response) {
        return rejectWithValue({
          message:
            axiosError.response.data.message || "Failed to fetch marketer data",
